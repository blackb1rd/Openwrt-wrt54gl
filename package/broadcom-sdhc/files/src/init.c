...
static int init_card_mm(struct card_info *c) {
    ...
    r = spi_card_cmd_r(58, 0x40000000, 0xff, ocr, 4 );
    if (r != R1_IDLE_STATE) {
        // An error occured - retry without high capacity support
        r = spi_card_cmd_r(58, 0x00000000, 0xff, ocr, 4 );
    }
    ...
    for (i = 0; i < 100000; i++) {
        r = spi_card_cmd_r(1, 0x00000000, 0xff, NULL, 0);
    ...
    r = spi_card_read_blk(10, 0x00000000, 0xff, cid, sizeof(cid));
    if (r != 0x00) {
    ...
    r = spi_card_read_blk(9, 0x00000000, 0xff, csd, sizeof(csd));
    if (r != 0x00) {
    ...
    if ( ((ocr[0] & 0x60) >> 5) == 2) {
        ...
        r = spi_card_read_blk(8, 0x00000000, 0xff, ext_csd, sizeof(ext_csd));
        if (r != 0x00) {
...
static int init_card_sd(struct card_info *c) {
    ...
    r = spi_card_cmd_r(8, 0x000001AA, 0x87, cic, 4 );  // CRC is checked for CMD8!
    if ((r & ~R1_IDLE_STATE) == 0) {
    ...
    for (i = 0; i < 100000; i++) {
        r = spi_card_cmd_r(55, 0x00000000, 0xff, NULL, 0);
        if (r != R1_IDLE_STATE) break;
        r = spi_card_cmd_r(41, cmd8 ? 0x70000000 : 0x00000000, 0xff, NULL, 0);
        if (r != R1_IDLE_STATE) break;
    }
    if ((r & R1_ILL_CMD) > 0) {
    ...
    r = spi_card_cmd_r(55, 0x00000000, 0xff, NULL, 0);
    r = spi_card_read_blk(51, 0x00000000, 0xff, scr, sizeof(scr));
    if (r != 0x00) {
...
static int init_card(struct card_info **ret) {
    ...
    r = spi_card_cmd_r(0, 0x00000000, 0x95, NULL, 0);
    if (r != 0x01 ) {
    ...
    r = spi_card_cmd_r(16, 512, 0xff, NULL, 0);
    if (r != 0x00 ) {
