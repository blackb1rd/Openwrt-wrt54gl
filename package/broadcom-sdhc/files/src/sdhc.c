 ...
 *     - Lexar SDHC - 16GB
 *
 *   No higher capacity MMC card tested - suspect possible problems at initialization.
...
#define DEVICE_NAME "sdcard"                            // Device name (messaging and in /dev).
...
#define VERSION "2.0.2"                                 // Module Version Number
...
#define LOG_INFO(...)           printk(KERN_INFO "[INF] sdhc: " __VA_ARGS__)
#define LOG_NOTICE(...)         printk(KERN_NOTICE "[NOT] sdhc: " __VA_ARGS__)
#define LOG_WARN(...)           printk(KERN_WARNING "[WRN] sdhc: " __VA_ARGS__)
#define LOG_ERR(...)            printk(KERN_ERR "[ERR] sdhc: " __VA_ARGS__)
#ifdef DEBUG
#define LOG_DEBUG(flag, ...)    if (dbg & flag) printk(KERN_INFO "[DBG] sdhc: " __VA_ARGS__)
#else
...
// Function prototypes
int sdhc_open(struct inode *inode, struct file *filp);
int sdhc_release(struct inode *inode, struct file *filp);
int sdhc_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg);
int sdhc_check_change(kdev_t);
int sdhc_revalidate(kdev_t);
static void sdhc_request(request_queue_t *);
static ssize_t sdhc_proc_read(struct file *, char *, size_t, loff_t *);
int sdhc_init(void);
void sdhc_exit(void);
...
static int sdhc_sizes[NUMDEV << PSHIFT];                // size of each device in kb
static int sdhc_blocksizes[NUMDEV << PSHIFT];           // size of block used by device
static int sdhc_hardsectsizes[NUMDEV << PSHIFT];        // size of sectors on device
static int sdhc_maxsect[NUMDEV << PSHIFT];              // max size of a single request
static struct hd_struct sdhc_part[NUMDEV << PSHIFT];    // decoded device partition table
static devfs_handle_t sdhc_devdir[NUMDEV];              // For managing our own devfs device directory


// Block device operations structure
static struct block_device_operations sdhc_bdops = {
        open:                   sdhc_open,
        release:                sdhc_release,
        ioctl:                  sdhc_ioctl,
//      check_media_change:     sdhc_check_change,
//      revalidate:             sdhc_revalidate,
};


// Generic disk support control structure
struct gendisk sdhc_gendisk = {
    ...
    part:               sdhc_part,                      // Pointer to partition global array
    sizes:              sdhc_sizes,                     // Pointer to sizes global array
    fops:               &sdhc_bdops,                    // Block dev operations
    de_arr:             sdhc_devdir                     // devfs device names
};
...
static struct file_operations sdhc_proc_fops = {
    read: sdhc_proc_read,
    write: NULL
};
...
int sdhc_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd) {
        case BLKGETSIZE:
            return put_user(sdhc_part[MINOR(inode->i_rdev)].nr_sects, (unsigned long *)arg);
        case BLKGETSIZE64:
            return put_user((u64)sdhc_part[MINOR(inode->i_rdev)].nr_sects, (u64 *) arg);
        case BLKRRPART:
            if (!capable(CAP_SYS_ADMIN)) return -EACCES;
            return sdhc_revalidate(inode->i_rdev);
        case HDIO_GETGEO:
            {
            ...
            g.heads = 4;
            g.sectors = 16;
            g.cylinders = sdhc_part[0].nr_sects / (4 * 16);
            g.start = sdhc_part[MINOR(inode->i_rdev)].start_sect;
 ...
 *-----------------------------------------------------------------------------*/
int sdhc_check_change(kdev_t i_rdev) {
 ...
 *-----------------------------------------------------------------------------*/
int sdhc_revalidate(kdev_t i_rdev) {
    int i, minor;
    ...
    for (i = (1 << PSHIFT) - 1; i >= 0; i--) {
        ...
        sdhc_gendisk.part[minor].start_sect = 0;
        sdhc_gendisk.part[minor].nr_sects = 0;
    }

    // Try to re-read partition table
    grok_partitions(&sdhc_gendisk, DEVICE_NR(i_rdev), (1 << PSHIFT), sdhc_part[0].nr_sects);
 ...
 *-----------------------------------------------------------------------------*/
static void sdhc_request(request_queue_t *q) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            part_sector = sdhc_part[MINOR(req->rq_dev)].start_sect;     // Starting sector of requested partition
            ...
            if (!(card->state & CARD_STATE_DET)) {
                ...
                code = 0;
            } else if ((part_sector + sector + nr_sectors) > sdhc_part[0].nr_sects) {
                LOG_ERR("Request: Sector out of range!\n");
                code = 0;
            } else if (cmd == READ) {
                spin_unlock_irq(&io_request_lock);
                r = spi_card_read_multi_o(card_address, buffer_address, nr_sectors, first, last);
                if (r != 0) {
                ...
                spin_unlock_irq(&io_request_lock);
                r = spi_card_write_multi_o(card_address, buffer_address, nr_sectors, first, last);
                if (r != 0) {
...
/*-----------------------------------------------------------------------------
 * Module /proc/sdhc/status read function
 *-----------------------------------------------------------------------------*/
static ssize_t sdhc_proc_read(struct file *file, char *buf, size_t count, loff_t *ppos) {
...
int sdhc_init(void) {
...
#ifdef DEBUG
    LOG_INFO("Version: " VERSION "  Parms: major=%d din=%d dout=%d clk=%d cs=%d maxsec=%d rahead=%d dbg=%d\n", major, din, dout, clk, cs, maxsec, rahead, dbg);
#else
    ...
    if (!(sdhcd = proc_mkdir(DEVICE_NAME, NULL))) {
    ...
    memset(sdhc_sizes, 0, sizeof(sdhc_sizes));
    memset(sdhc_part, 0, sizeof(sdhc_part));
    for (i = 0; i < (1 << PSHIFT); i++) sdhc_maxsect[i] = maxsec;
    sdhc_sizes[0] = card->blocks / 2;           // number of 1024 byte blocks
    sdhc_part[0].nr_sects = card->blocks;       // number of 512  byte sectors
    read_ahead[MAJOR_NR] = rahead;
    blk_size[MAJOR_NR] = sdhc_sizes;
    blksize_size[MAJOR_NR] = sdhc_blocksizes;
    ...
    add_gendisk(&sdhc_gendisk);

    // Register the disk - system will now read the partition table using sdhc_request
    register_disk(&sdhc_gendisk, MKDEV(MAJOR_NR,0), (1 << PSHIFT), &sdhc_bdops, sdhc_part[0].nr_sects);
...
void sdhc_exit(void) {
    ...
    del_gendisk(&sdhc_gendisk);
    devfs_unregister(sdhc_devdir[0]);
...
MODULE_DESCRIPTION("SD/SDHC/MMC Card Block Driver - " VERSION);
